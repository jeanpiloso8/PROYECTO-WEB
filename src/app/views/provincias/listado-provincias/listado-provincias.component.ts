...
export class ListadoProvinciasComponent {
  ...
  cargarRegistros(): void {
    ...
    this.provinciasService.todos().subscribe({
      next: (respuesta) => {
  ...
  public commandClick(args: any): void {
    ...
    {
      const id = args.rowData.idProvincia;
      this.router.navigate(['/provincia/modificar', id]);
    }
  ...
  async eliminar(args: any) {
    ...
    if (result.isConfirmed) {
      this.spinnerService.showGlobalSpinner();
      try {
        const respuesta: any = await firstValueFrom(this.provinciasService.borrar(args.rowData.idProvincia));
  ...
  clickHandler(args: ClickEventArgs): void {
    ...
    if (args.item.id === 'Nuevo') {
      this.router.navigate(['/provincia/nuevo']);
    }
  }
