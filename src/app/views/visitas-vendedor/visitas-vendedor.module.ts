import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';
import { ListadoVisitasComponent } from './listado-visitas/listado-visitas.component';
import { ListadoImagenesComponent } from './listado-imagenes/listado-imagenes.component';
import { ListadoAudioComponent } from './listado-audio/listado-audio.component';
import { VisitasVendedorRoutingModule } from './visitas-vendedor-routing.module';
import { ReactiveFormsModule } from '@angular/forms';
import { AccordionModule, CardModule, FormModule, HeaderModule, SharedModule, UtilitiesModule , GridModule as GridCoreUI} from '@coreui/angular';
import { IconModule, IconSetService } from '@coreui/icons-angular';
import { AggregateService, CommandColumnService, EditService, ExcelExportService, GridModule, GroupService, LazyLoadGroupService, PageService, PdfExportService, ResizeService, SearchService, SortService, ToolbarService } from '@syncfusion/ej2-angular-grids';
import { ComboBoxModule } from '@syncfusion/ej2-angular-dropdowns';
import { ToolbarModule } from '@syncfusion/ej2-angular-navigations';



@NgModule({
  declarations: [ListadoVisitasComponent,ListadoImagenesComponent,ListadoAudioComponent],
  imports: [
    CommonModule,
    VisitasVendedorRoutingModule,
    ReactiveFormsModule,
    FormModule,
    IconModule,
    UtilitiesModule,
    AccordionModule,
    SharedModule,
    GridModule,
    CardModule,
    HeaderModule,
    ToolbarModule,
    ComboBoxModule,
    GridCoreUI,
    
  ],
  providers:[IconSetService,
    AggregateService, GroupService, EditService, ToolbarService, SortService, PageService,CommandColumnService,LazyLoadGroupService,SearchService,ResizeService,
    ExcelExportService,PdfExportService]
})
export class VisitasVendedorModule { }
